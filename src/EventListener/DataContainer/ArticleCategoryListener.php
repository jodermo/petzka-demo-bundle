...
use Petzka\DemoBundle\MultilingualHelper;
...
class ArticleCategoryListener implements FrameworkAwareInterface
{
    ...
    /**
     * ArticleCategoryListener constructor.
     *
     * @param Connection        $db
    ...
    public function onLoadCallback(DataContainer $dc)
    {
        if (!$this->permissionChecker->canUserManageCategories() && !$this->permissionChecker->canUserAssignCategories()) {
            throw new AccessDeniedException('User has no permissions to manage article categories');
        }
        ...
        // Get the article categories root set previously in session (see #137)
        if (isset($_SESSION['ARTICLE_CATEGORIES_ROOT']) && $input->get('picker')) {
            $GLOBALS['TL_DCA'][$dc->table]['list']['sorting']['root'] = $_SESSION['ARTICLE_CATEGORIES_ROOT'];
        ...
        if (null !== ($roots = $this->permissionChecker->getUserAllowedRoots())) {
            ...
            switch ($action = $input->get('act')) {
                case 'edit':
                    ...
                    if (!$this->permissionChecker->isUserAllowedArticleCategory($categoryId)) {
                ...
                case 'show':
                    ...
                    if (!$this->permissionChecker->isUserAllowedArticleCategory($categoryId)) {
                        throw new AccessDeniedException(\sprintf('Not enough permissions to %s article category ID %s.', $action, $categoryId));
                    }
                    break;
                case 'editAll':
                case 'deleteAll':
