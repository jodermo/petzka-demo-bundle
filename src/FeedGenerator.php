...
class FeedGenerator extends Article
{
    /**
     * @inheritDoc
     */
    protected function generateFiles($arrFeed)
    {
        ...
        try {
            ...
            $objArticle = \ArticleModel::findBy($criteria->getColumns(), $criteria->getValues(), $criteria->getOptions());
        ...
        if ($objArticle !== null)
        {
            ...
            while ($objArticle->next())
            {
                ...
                // Get the categories
                if ($arrFeed['categories_show'] && ($categoryModels = ArticleCategoryModel::findPublishedByArticle($objArticle->id)) !== null) {
                ...
                {
                    $strDescription = '';
                    $objElement = \ContentModel::findPublishedByPidAndTable($objArticle->id, 'tl_article');
                ...
                if (count($categories) > 0) {
                    switch ($arrFeed['categories_show']) {
                        ...
                        case 'text_before':
                        case 'text_after':
                            $buffer = sprintf('<p>%s %s</p>', $GLOBALS['TL_LANG']['MSC']['articleCategories'], implode(', ', $categories));
