...
class ArticleCategoriesPickerWidget extends Widget
{
    /**
     * Submit user input.
     *
    ...
    public function generate()
    {
        ...
        // Can be an array
        if (!empty($this->varValue) && null !== ($categories = ArticleCategoryModel::findMultipleByIds((array) $this->varValue))) {
  ...
  <div class="selector_container">'.(($this->sorting && count($values) > 1) ? '
    ...
    <ul id="sort_'.$this->strId.'" class="'.($this->sorting ? 'sortable' : '').'">';
        ...
        if (!$pickerBuilder->supportsContext('articleCategories')) {
            $return .= '
	<p><button class="tl_submit" disabled>'.$GLOBALS['TL_LANG']['MSC']['changeSelection'].'</button></p>';
            ...
            $return .= '
	<p><a href="'.ampersand($pickerBuilder->getUrl('articleCategories', $extras)).'" class="tl_submit" id="pt_'.$this->strName.'">'.$GLOBALS['TL_LANG']['MSC']['changeSelection'].'</a></p>
	<script>
	  $("pt_'.$this->strName.'").addEvent("click", function(e) {
		...
		Backend.openModalSelector({
		  ...
		  "callback": function(table, value) {
			...
			}).post({"action":"reloadArticleCategoriesWidget", "name":"'.$this->strId.'", "value":value.join("\t"), "REQUEST_TOKEN":"'.REQUEST_TOKEN.'"});
		  }
		});
	  });
	</script>'.($this->sorting ? '
    ...
    protected function checkValue($input)
    {
        ...
        if (count(array_diff($ids, array_merge($this->rootNodes, Database::getInstance()->getChildRecords($this->rootNodes, 'tl_article_category')))) > 0) {
